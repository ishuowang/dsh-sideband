...
export type SidebandCommand =
  ...
  | { readonly kind: 'status'; readonly jobId?: string }
...
export function tokenizeSidebandInput(input: string): string[] {
  const tokens: string[] = []
  let current = ''
  let quote: '"' | "'" | undefined
  let escaped = false
  ...
  if (escaped) throw new SidebandCommandError('input ends with an unfinished escape')
  if (quote !== undefined) throw new SidebandCommandError('input contains an unterminated quote')
  if (started) tokens.push(current)
  return tokens
}

function targetFrom(value: string, maximum: number): SidebandTarget {
  if (value.length > maximum) throw new SidebandCommandError(`target exceeds ${maximum} characters`)
  ...
  if (/\s/u.test(id)) throw new SidebandCommandError('target id cannot contain whitespace')
  return { kind, id }
}

function flagValue(tokens: readonly string[], index: number, flag: string): { value: string; next: number } {
  const token = tokens[index]
  if (token === undefined) throw new SidebandCommandError(`missing ${flag} value`)
  const equals = token.indexOf('=')
  if (equals >= 0) {
    const value = token.slice(equals + 1)
    if (value.length === 0) throw new SidebandCommandError(`missing ${flag} value`)
...
function parsePositiveCount(value: string, maximum: number): number {
...
function parseSend(tokens: readonly string[], options: ParseOptions): SidebandCommand {
  const targetToken = tokens[0]
  if (targetToken === undefined) throw new SidebandCommandError('send requires a target')
  const target = targetFrom(targetToken, options.maxTargetChars ?? 512)
  let scope: SidebandScope = { kind: 'last', count: options.defaultLastMessages }
  let delivery: SidebandDelivery = 'quiet'
  ...
  for (let index = 1; index < tokens.length;) {
    ...
    if (token === '--all' || token === '--full') {
      if (scopeSeen) throw new SidebandCommandError('choose only one of --all and --last')
      scope = { kind: 'all' }
      scopeSeen = true
      index += 1
      continue
    ...
    if (token === '--focus' || token?.startsWith('--focus=') === true) {
      if (focus !== undefined) throw new SidebandCommandError('--focus may be supplied only once')
      const parsed = flagValue(tokens, index, '--focus')
      const normalized = parsed.value.trim()
      if (normalized.length === 0) throw new SidebandCommandError('--focus cannot be empty')
...
export function parseSidebandCommand(rawInput: string, options: ParseOptions): SidebandCommand {
  const tokens = tokenizeSidebandInput(rawInput)
  const verb = tokens[0]
  if (verb === undefined || verb === 'help') {
    if (tokens.length > 1) throw new SidebandCommandError('help accepts no arguments')
    return { kind: 'help' }
  }
  if (verb === 'status') {
    if (tokens.length > 2) throw new SidebandCommandError('status accepts at most one job id')
  ...
  if (verb === 'targets') {
    if (tokens.length !== 1) throw new SidebandCommandError('targets accepts no arguments')
    return { kind: 'targets' }
  }
  if (verb === 'send') return parseSend(tokens.slice(1), options)
  if (verb.startsWith('session:') || verb.startsWith('room:')) return parseSend(tokens, options)
