...
function commandBoundary(events, commandId) {
    for (let index = events.length - 1; index >= 0; index -= 1) {
        const event = events[index];
        if (event?.type === 'command/run' && String(event.data.commandId) === String(commandId))
            return event.seq;
    }
...
function visibleEntries(messages) {
    ...
    for (const message of messages) {
        ...
        if (!isHumanOrModelMessage(message))
            continue;
...
export function openAssistantText(events) {
    let openTurn;
    let openStep;
    ...
    if (stepEvents.some(event => event.type === 'assistant/message'))
        return '';
    let attemptStart = 0;
    ...
    for (const event of stepEvents.slice(attemptStart)) {
        ...
        if (chunk.type === 'block-start') {
            if (!blocks.has(chunk.index))
                blocks.set(chunk.index, { order: order++, text: '', kind: chunk.blockType });
        }
        else if (chunk.type === 'text-delta') {
            const block = blocks.get(chunk.index) ?? { order: order++, text: '', kind: 'text' };
            if (block.kind === undefined || block.kind === 'text')
...
function prefixMessages(session, events, boundarySeq) {
    if (events.at(-1)?.seq === boundarySeq)
...
function applyScope(entries, scope) {
    return scope.kind === 'all' ? [...entries] : entries.slice(-scope.count);
}
function boundEntries(entries, maximum) {
    const selected = [];
...
export function captureConversationSnapshot(options) {
    ...
    if (partialText.length > 0)
        entries.push({ role: 'assistant', text: partialText, partial: true });
