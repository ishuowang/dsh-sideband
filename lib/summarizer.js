...
export function resolveSummaryRoute(config, agent) {
    ...
    if (route !== undefined)
        return route;
    throw new Error('sideband: no provider/model available; configure Sideband provider+model or route the source Session first');
}
export function renderSnapshot(snapshot, focus) {
...
function terminalError(assembler) {
...
export async function summarizeJob(ctx, config, job, workerSignal) {
    ...
    const summary = blocks
        ...
        .trim();
    if (summary.length === 0)
        throw new Error('sideband summarizer: model produced no visible text');
